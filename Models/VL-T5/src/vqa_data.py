...
class VQAFineTuneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_ids = self.tokenizer.encode(f'chartqa: {sent}', max_length=400, truncation=True) #changed 20 to 400.
    ...
    def collate_fn(self, batch):
        ...
        if args.use_vision:
            V_L = 36#len(batch[0]['boxes']) For Padding!
...
class VQAEvaluator:
    def __init__(self, src_folder="data/train"):
        ...
        self.qidtoans = {}
        for id, ans in zip(self.questions_ids, self.outputs):
            self.qidtoans[id] = ans
