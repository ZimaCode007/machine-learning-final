...
def get_final_answer(df, selected_cells, operator, fixed_vocab):
    if operator > 6:
        return fixed_vocab[operator-6]
    elif operator == 0: #Select
        try:
            return [df.iloc[x[0], x[1]] for x in selected_cells]
        except:
            return 0
    elif operator == 1: #SUM:
        try:
            return sum([float(df.iloc[x[0], x[1]]) for x in selected_cells])
        except:
            return 0
    elif operator == 2: #Average:
        try:
            lst = [float(df.iloc[x[0], x[1]]) for x in selected_cells]
            return sum(lst) / len(lst)
        except:
            return 0
    elif operator == 3: #Count
        return len(selected_cells)
    elif operator == 4: #Diff
        try:
            x1 = float(df.iloc[selected_cells[0][0], selected_cells[0][1]])
            y1 = float(df.iloc[selected_cells[1][0], selected_cells[1][1]])
            return max(x1, y1) - min(x1, y1)
        except:
            return 0
    elif operator == 5: #Ratio
        try:
