...
def _single_column_cell_selection_loss(token_logits, column_logits, labels, cell_index, col_index, cell_mask):
    ...
    column_dist = torch.distributions.Categorical(logits=column_logits, validate_args=False)  # shape (batch_size, max_num_cols)
    column_loss_per_example = -column_dist.log_prob(column_label)
    #print(column_label)
    #print("Column loss:", column_loss_per_example)
    # Part 2: cell loss
...
def _calculate_aggregate_mask(answer, pooled_output, cell_selection_preference, labels, aggregation_classifier):
    ...
    is_pred_cell_selection = aggregation_ops_total_mass <= cell_selection_preference # <= to <
...
def _calculate_aggregation_loss_known(
    logits_aggregation, aggregate_mask, aggregation_labels, use_answer_as_supervision, num_aggregation_labels, class_labels_mask, class_labels
):
    """
    ...
    if use_answer_as_supervision:
        ...
        target_aggregation = class_labels #torch.zeros_like(aggregate_mask, dtype=torch.long)
    else:
...
def _calculate_aggregation_loss_unknown(logits_aggregation, aggregate_mask):
    ...
    dist_aggregation = torch.distributions.categorical.Categorical(logits=logits_aggregation, validate_args=False)
    # Index 0 corresponds to "no aggregation".
    aggregation_ops_total_mass = torch.sum(dist_aggregation.probs[:, 1:4], dim=1)
...
def _calculate_expected_result(
    ...
    if config.use_gumbel_for_aggregation:
        gumbel_dist = torch.distributions.RelaxedOneHotCategorical(
            config.aggregation_temperature, logits=logits_aggregation[:, 1:4]
        )
        ...
        aggregation_op_only_probs = nn.functional.softmax(
            logits_aggregation[:, 1:4] / config.aggregation_temperature, dim=-1
        )
