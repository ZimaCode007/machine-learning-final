...
class VisionTapasForQuestionAnsweringDataset(torch.utils.data.Dataset):
    ...
    def _try_get_item(self, idx):
        ...
        if answer_type == 'FIXED/OPEN':
            if len(answer_list) == 1 and answer_list[0].lower() in self.classes_mappings:
                ...
                class_labels_mask = 1
                answer_texts = [str(data_table.iloc[0, 0])]
            else:
                class_labels = 0
                class_labels_mask = 0
    ...
    def __getitem__(self, idx):
        import random
        try:
            return self._try_get_item(idx)
        except (ValueError, KeyError, FileNotFoundError, IndexError):
            for _ in range(10):
                alt = random.randint(0, len(self.instances) - 1)
                try:
                    return self._try_get_item(alt)
                except (ValueError, KeyError, FileNotFoundError, IndexError):
                    continue
            raise RuntimeError(f"Cannot find any valid sample near index {idx}")
